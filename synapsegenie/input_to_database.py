...
def check_existing_file_status(validation_status_table, error_tracker_table, entity):
    """
    ...
    if current_status.empty:
        ...
        if (
            current_status["md5"].values[0] != entity.md5
            or current_status["name"].values[0] != entity.name
        ):
            to_validate = True
        else:
    ...
    return {"status_list": status, "error_list": error, "to_validate": to_validate}
...
def validation(
    ...
    validation_status_synid = process_functions.get_database_synid(
        syn, "validationStatus", database_mappingdf=database_synid_mappingdf
    )
    error_tracker_synid = process_functions.get_database_synid(
        syn, "errorTracker", database_mappingdf=database_synid_mappingdf
    )
...
def center_input_to_database(
    ...
    if only_validate:
        ...
        log_path = os.path.join(process_functions.SCRIPT_DIR, f"{center}_log.txt")
    ...
    if delete_old:
        ...
        shutil.rmtree(os.path.join(path_to_genie, center), ignore_errors=True)
