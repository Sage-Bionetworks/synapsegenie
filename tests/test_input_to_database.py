...
def test_valid_validatefile():
    ...
    with patch.object(
        ValidationHelper, "determine_filetype", return_value=filetype
        ...
        ValidationHelper, "validate_single_file", return_value=(valid, message)
    ) as patch_validate, patch.object(
        input_to_database,
        "_get_status_and_error_list",
        return_value=status_error_list_results,
    ) as patch_get_staterror_list, patch.object(
        ...
        patch_get_staterror_list.assert_called_once_with(valid, message, entity)
...
def test_already_validated_validatefile():
    ...
    with patch.object(
        ...
        ValidationHelper, "validate_single_file", return_value=(valid, errors)
    ) as patch_validate, patch.object(
        input_to_database,
        "_get_status_and_error_list",
        return_value=status_error_list_results,
...
def test_valid__get_status_and_error_list():
    ...
    (
        input_status_list,
        invalid_errors_list,
    ) = input_to_database._get_status_and_error_list(valid, message, entity)
...
def test_invalid__get_status_and_error_list():
    ...
    (
        input_status_list,
        invalid_errors_list,
    ) = input_to_database._get_status_and_error_list(valid, errors, entity)
...
def test__send_validation_error_email():
    ...
    with patch.object(
        ...
        "synapsegenie.input_to_database.datetime"
    ) as mock_datetime:
        ...
        self.with_dupsdf = pd.DataFrame(
            {
                ...
                "name": ["test.txt", "test.txt", "test.txt", "test.txt", "test.txt"],
                "center": ["SAGE"] * 5,
                "fileType": ["type"] * 5,
                "entity": ["entity"] * 5,
            }
...
def test_main_processfile(genieclass, filetype):
    ...
    path_to_genie = "./"
    center_mapping = {"stagingSynId": ["syn123"], "center": [center]}
    center_mapping_df = pd.DataFrame(center_mapping)
    databaseToSynIdMapping = {"Database": [filetype], "Id": ["syn222"]}
    databaseToSynIdMappingDf = pd.DataFrame(databaseToSynIdMapping)
...
def test_mainnone_processfile():
    ...
    center_mapping = {"stagingSynId": ["syn123"], "center": [center]}
    center_mapping_df = pd.DataFrame(center_mapping)
    databaseToSynIdMapping = {"Database": ["clinical"], "Id": ["syn222"]}
    databaseToSynIdMappingDf = pd.DataFrame(databaseToSynIdMapping)
    process_cls = Mock()
