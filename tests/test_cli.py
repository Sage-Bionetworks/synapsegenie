...
def test_perform_validate():
    ...
    with patch.object(
        ...
        process_functions, "get_dbmapping", return_value={"df": arg.asDataFrame()}
    ) as patch_getdb, patch.object(
        syn, "tableQuery", return_value=arg
    ) as patch_syn_tablequery, patch.object(
        validate, "_check_center_input"
    ) as patch_check_center, patch.object(
        config, "collect_validation_helper", return_value=validate.ValidationHelper
    ) as patch_val_col, patch.object(
        config, "collect_format_types"
    ) as patch_collect, patch.object(
        validate.ValidationHelper, "validate_single_file", return_value=(valid, "foo")
